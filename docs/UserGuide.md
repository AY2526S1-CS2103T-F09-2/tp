...
4. There are a few different ways to launch the application :
   ...
   - _(For advanced command app users)_ Open a command terminal, `cd` into the folder you put the jar file in, and use the `java -jar addressbook.jar` command to run the application.<br>
...
This ensures your recurring lessons always reflect the _next upcoming_ session without requiring manual changes.
...
- When a student is initialised, _by default_ :
...
- If you want to add a **recurring lesson**, a lesson that refreshes itself after a fixed number of days, you can use the optional `every/` identifier with a **positive integer** to indicate after how many days the lesson will automatically update itself to the next date instead of deleting itself. When you do not provide the `every/` identifier, the lesson will be counted as a normal lesson, which automatically deletes itself after the date of the lesson has passed.

- Note: You may provide the `every/` identifier at most once. Providing `every/` more than once (for example `addLesson 1 d/2025-11-13 every/7 every/14`) will result in an error and the lesson will not be added.
...
| **Add Lesson**     | `addLesson INDEX d/LESSONDATE [every/INTERVAL]` <br> (only one `every/INTERVAL` allowed) <br> e.g. `addLesson 2 d/2025-11-13`                                                                             |
